class UltimateWeightedWheel {
  constructor(startingPoints = 100, startingSpins = 5) {
    ...
    this.pointsWeights = [
      0.2, 0.16, 0.14, 0.12, 0.1, 0.08, 0.06, 0.04,
      0.03, 0.02, 0.015, 0.01, 0.008, 0.006, 0.005, 0.004, 0.002,
    ];
  ...
  rollNestedFreeSpins() {
    return this.weightedChoice(this.spinsPayoutList, this.spinsPayoutWeights);
  }
  ...
  spin() {
    ...
    const landedOutcome = this.weightedChoice(this.wheelOutcomes, this.wheelWeights);

    if (landedOutcome === 'POINTS') {
      const pointsWon = this.rollDiscretePoints();
      this.points += pointsWon;
      console.log(`🎯 Outcome: Standard Points! Added +${pointsWon.toLocaleString()} to your score.`);
      ...
      console.log(`    Rolled a [${multiplier}x] Modifier! (${basePayout} Base Points x ${multiplier}x = +${pointsWon.toLocaleString()} points!)`);
      ...
      console.log(`    Rolled an Extra Turn token ➡️ Granted +${spinsWon} Remaining Spins!`);
    } else if (landedOutcome === 'DOUBLE') {
      const oldPoints = this.points;
      this.points *= 2;
      console.log(`💥 JACKPOT: DOUBLE CURRENT POINTS! Score shifted from ${oldPoints.toLocaleString()} to ${this.points.toLocaleString()}!`);
    }
  ...
  displayHud() {
    ...
    console.log(`📊 POINTS TOTAL: ${this.points.toLocaleString()}  |  🎟️ REMAINING SPINS: ${this.freeSpins}`);
    console.log('-'.repeat(70));
  }
}
